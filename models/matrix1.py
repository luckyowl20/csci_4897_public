...
class SIRModelMatrix:

    def __init__(self, s0, i0, r0, pC, gamma, tmax, stepsize):
        """
        ...
        # probability of transmission per contact matrix
        self.pC = pC
        self.gamma = gamma
        self.tmax = tmax
        ...
        # tihs might not be needed anymore
        self.N = s0 + i0 + r0

        # Time points
        self.time = np.arange(0, tmax + stepsize, stepsize)

        # need changes here for and I, R
        # S = np.zeros([len(T), G])
    ...
    def run_model(self):
        for idx, t in enumerate(self.time):
            if idx == 0:
                ...
                # find N for the groups we care about
                # N = S[0, :] + I[0, :] + R[0, :]
            else:
                # more changes here, creating a helper matrix
                # this gives us the total number of contacts between groups, not proportions like class notes
                ...
                # update equations for eulers method, matrix form using [idx-1, :]
